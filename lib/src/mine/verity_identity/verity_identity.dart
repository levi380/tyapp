import 'package:filbet/src/mine/verity_identity/email_verity/email_verity.dart';
import 'package:filbet/src/mine/verity_identity/phone_verity/phone_verity.dart';
...
class VerityIdentity extends GetView<VerityIdentityController> {
  @override
            ...
            Obx(() {
              return Tab(
                child: Text(
                  ...
                  style: TextStyle(
                    color: controller.selectTabIndex.value == i ? Color(0xff1D1D1F) : Color(0xffB0B1B6),
                    fontSize: 16.sp,
                  ),
                ),
              );
  ...
  List<Widget> _buildTabBarViewChildren() {
    ...
    if (controller.phone?.isNotEmpty == true) {
      children.add(PhoneVerity(
          verityType: controller.verityType ?? ValidationType.changeLoginPwd, accountId: controller.accountId));
    }

    // 如果需要邮箱验证，添加邮箱验证页面
    if (controller.email?.isNotEmpty == true) {
      children.add(EmailVerity(verityType: controller.verityType ?? ValidationType.changeLoginPwd, accountId: controller.accountId));
    }
