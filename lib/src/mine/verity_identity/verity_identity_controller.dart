...
class TabItem {
  ...
  TabItem(this.title, {this.imageIcon = "", this.hasRedPoint = false, this.isNetImage = false});
}

class VerityIdentityController extends GetxController with GetSingleTickerProviderStateMixin {
  List<TabItem> tabs = [];
  ...
  void onInit() {
    ...
    if (arguments != null) {
      ...
      if (Get.arguments.containsKey('accountId')) {
        accountId = arguments['accountId'];
      }

      verityType = arguments['verityType'];
    }
    // 根据判断结果动态设置 tabs
    tabs = [];
    ...
    tabController = TabController(length: tabs.isEmpty ? 1 : tabs.length, vsync: this);
  }
