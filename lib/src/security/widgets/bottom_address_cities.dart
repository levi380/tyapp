...
class BottomAddressCities extends StatefulWidget {
  final ValueChanged<AddressCitiesModel> onSelect;
  RxList<AddressCitiesModel> cities;

  BottomAddressCities({super.key, required this.cities, required this.onSelect});

  @override
  State<BottomAddressCities> createState() => _BottomAddressCities();
}

class _BottomAddressCities extends State<BottomAddressCities> {
  List<AddressCitiesModel> filterList = [];
  TextEditingController searchController = TextEditingController();

  @override
  void initState() {
    super.initState();
    filterList = List.from(widget.cities);
    searchController.addListener(() {
      if (searchController.text.isEmpty) {
        ...
        setState(() {
          filterList = widget.cities.where((e) => e.name?.contains(searchController.text) ?? false).toList();
        });
      }
    });
  }
  ...
  Widget itemView(AddressCitiesModel item) {
    return InkWell(
      child: Container(
        height: 44.h,
