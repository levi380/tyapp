...
class MessageCenterListView extends GetView<MessageCenterListController> {
  const MessageCenterListView({super.key});

  @override
  Widget build(BuildContext context) {
    if (!Get.isRegistered<MessageCenterListController>()) {
      final initialIndex = Get.find<MessageCenterController>().initialMessageTabIndex;
      Get.put(MessageCenterListController(initialTabIndex: initialIndex));
    }
    return Column(
      children: [
        tabView(),
  ...
  Widget dataView() {
    return TabBarView(
      ...
      children: [
        ...
        _buildMessagePage(2, controller.announceModels), // 公告
        _buildMessagePage(3, controller.depositWithdrawModels), // 充提
      ],
    );
  }
