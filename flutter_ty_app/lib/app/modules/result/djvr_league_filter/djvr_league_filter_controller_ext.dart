 ...
 *
 */
extension DjVrLeagueFilterControllerExt on DjVrLeagueFilterController {
  ...
  onObserves(index) {
    if (logic.click == true) {
      ...
      /// 使用 map 方法遍历每个字符串，并去掉头尾空格
      keywords = keywords.map((keyword) => keyword.trim()).toList();
      for (int i = 0; i < getFilterMatchListNews[0].tournaments.length; i++) {
        if (i == index) {
          logic.spell = getFilterMatchListNews[0].tournaments[i].spell;
        }
      }
  ...
  onSelectItem(id) {
    ...
    /// 判断子 item 是否选中
    logic.selectAll = getFilterMatchListNews.every((item) {
      return item.isSelect &&
          item.tournaments.every((tournament) => tournament.isSelect);
    });
  ...
  void changeIndex(String indexId) {
    ...
    for (int i = 0; i < getFilterMatchListNews[0].tournaments.length; i++) {
      if (getFilterMatchListNews[0].tournaments[i].id == indexId) {
        logic.spell = getFilterMatchListNews[0].tournaments[i].spell;
