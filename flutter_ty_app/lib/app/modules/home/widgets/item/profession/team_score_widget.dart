...
class TeamScoreWidget extends StatelessWidget {
  ...
  String _calculateScore() {
    bool isSpecialPlay = [
      ...
      playIdConfig.hps6Minutes,
      playIdConfig.hpsSection1,
      playIdConfig.hpsSection2,
      playIdConfig.hpsSection3,
      playIdConfig.hpsSection4,
      playIdConfig.hpsFirstHalf,
      playIdConfig.hpsSecondHalf,
    ].contains(playId);
    ...
    if (isSpecialPlay) {
      ...
      if (int.parse(matchEntity.csid) == SportData.sportCsid_5) {
        ...
        if ((matchEntity.csid == '1' || matchEntity.csid == '11') && (matchEntity.mmp == '32' || matchEntity.mmp == '33' || matchEntity.mmp == '41' || matchEntity.mmp == '42' || matchEntity.mmp == '110')){
          return FormatScore.formatTotalScoreOvertime(matchEntity, index, playId: playId);
        }else{
          /// 34 等待点球大战
          /// 50 点球大战进行中
          if ((matchEntity.csid == '1' || matchEntity.csid == '11') && (matchEntity.mmp == '34' || matchEntity.mmp == '50' )){
  ...
  Widget build(BuildContext context) {
    ...
    final subTex = "${!isMainPlay ?
    " - ${TyHomeController.to.getSecondaryName(matchEntity.mid)}" :
    ""}"
        "${(matchEntity.mmp == '32' || matchEntity.mmp == '33' || matchEntity.mmp == '42' || matchEntity.mmp == '110') ?
    ' - ${LocaleKeys.analysis_overtime.tr}' : ''
        "${(matchEntity.mmp == '34' || matchEntity.mmp == '50') ?
