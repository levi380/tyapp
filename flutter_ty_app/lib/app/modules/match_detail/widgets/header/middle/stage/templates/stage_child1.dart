 ...
 *
    ...
    ```
    // 补时
    if (["6", "7"].contains(widgets.match.mmp) &&
    ObjectUtil.isNotEmpty(widgets.match.mststi) &&
    widgets.match.mststi != '0')
    ```
    】】】
 *
 */
...
class _StageChild1State extends State<StageChild1> {
  ...
  Widget build(BuildContext context) {
    if (matchResultState()) {
      ...
      String countingTimeFormatStr =
          FormatDate.countingTimeCtrShowFormat(widget.match, countingTime);
      bool showCountingTime = showTimeCounting();
      return Column(
        ...
        children: [
          ...
          // 非置顶或者显示倒计时
          if (!widget.isPinnedAppbar || showCountingTime)
            RichText(
              maxLines: 2,
              ...
              text: TextSpan(children: [
                if (!widget.isPinnedAppbar)
                  TextSpan(
                    text: "mmp_1_${widget.match.mmp}".tr,
                    style: TextStyle(
                      color: widget.isMatchSelect
                          ? Get.theme.subSelectTitleColor
                          : Colors.white,
                ...
                if (![31].contains(widget.match.mmp))
                  if (showCountingTime)
                    TextSpan(
                      text: widget.timeLineWrapDisplay ? "\n$countingTimeFormatStr" :  " $countingTimeFormatStr",
                      style: TextStyle(
                        color: widget.isMatchSelect
                            ? Get.theme.subSelectTitleColor
                            : Colors.white,
                        fontSize: widget.isPinnedAppbar
                            ...
                            : Get.theme.fontSize_12,
                        fontFamily:
                            widget.isPinnedAppbar ? 'DIN' : 'PingFang SC',
                ...
                if (["6", "7"].contains(widget.match.mmp) &&
                  ...
                  TextSpan(
                    text: "+${widget.match.mststi}'",
                    style: TextStyle(
                      // color: widget.isMatchSelect
                      //     ? Get.theme.subSelectTitleColor
                      //     : const Color.fromRGBO(204, 221, 255, 1),
                      color: widget.isPinnedAppbar
