...
class TDHorizontalTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// This widgets's selection and animation state.
  ///
  ...
  /// [TDHorizontalTabBar] in a [Padding] widgets. When [isScrollable] is true, the scrollable itself is inset,
  ...
  /// when the [indicatorColor] is same as [Material.color] of the [Material] parent widgets.
  ...
  /// (the default) or relative to the bounds of the tab's widgets if
...
class _TDHorizontalTabBarState extends State<TDHorizontalTabBar> {
  ...
  void initState() {
    ...
    // the width of tab widgets i. See _IndicatorPainter.indicatorRect().
  ...
  void _handleTap(int index) {
    ...
    // 前置检查
    if (widget.onPreCheck != null) {
      final allow = widget.onPreCheck!(index);
      if (!allow) return; // 不允许点击，直接返回
    }
  ...
  BoxDecoration? _getContentDecorateInner(int index) {
    if (widget.outlineType == AnalysisTabBarOutlineType.capsule) {
    ...
    } else if (widget.outlineType == AnalysisTabBarOutlineType.slider) {
      return BoxDecoration(
        color: index == _currentIndex
            ? (widget.selectedBgColor ?? Colors.blue)
            : null,
  ...
  Color? _getBackgroundColor(int index) {
    if (widget.outlineType == AnalysisTabBarOutlineType.slider) {
      if (index == _currentIndex) {
    ...
    final wrappedTabs = List<Widget>.generate(widget.tabs.length, (int index) {
      ...
      return Container(
        decoration: BoxDecoration(
          ...
          borderRadius: widget.outlineType == AnalysisTabBarOutlineType.slider
          ...
          children: [
            Container(
              ...
              child: Center(
                heightFactor: 1.0,
                child: Padding(
                  padding: widget.outlineType ==
                          AnalysisTabBarOutlineType.slider
                      ? (widget.selectedBgColor == widget.unSelectedBgColor &&
                              !widget.isScrollable
                          ? EdgeInsets.zero
                          : (widget.labelPadding ?? kTabLabelPadding))
                      : adjustedPadding ??
                          widget.labelPadding ??
            ...
            // 右上角氣泡提示
            if (widget.tabs[index].badgeText != null)
              Transform.translate(
                offset: Offset(10.w, -20.h), // ← 從中間右移 50
                child: Container(
                  padding: EdgeInsets.symmetric(horizontal: 5.w),
                  constraints: BoxConstraints(minWidth: 12.w, minHeight: 13.h),
                  decoration: BoxDecoration(
                    image: DecorationImage(
                      image: AssetImage(
                        widget.tabs[index].badgeText!.length > 1
                            ? 'assets/images/icons/message-center/badge-bg2.png'
                            : 'assets/images/icons/message-center/badge-bg.png',
                      ),
                      fit: BoxFit.fill,
                    ),
                  ),
                  child: Text(
                    widget.tabs[index].badgeText!,
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 10.sp,
                      fontFamily:
                          num.tryParse(widget.tabs[index].badgeText!) != null
                              ? 'Noto Sans TC'
                              : 'Inter',
              ...
              Positioned(
                ...
                child: Builder(
                  builder: (context) {
                    ...
                    bool isPrev = index == ((_currentIndex ?? 0) - 1);
                    bool isCurrent = index == _currentIndex;

                    bool shouldShow = (widget.outlineType ==
                            AnalysisTabBarOutlineType.slider &&
                        widget.selectedBgColor != null &&
                        widget.unSelectedBgColor != null &&
                        widget.selectedBgColor == widget.unSelectedBgColor &&
                        !widget.isScrollable);
                    return Container(
                      clipBehavior: Clip.none,
                      margin: EdgeInsets.symmetric(
                        vertical:
                            _currentIndex == index ? 0 : (widget.gap ?? 8.h),
                      ),
                      width: shouldShow && !isLast
                          ? 1
                          : (isLast || isPrev || isCurrent ? 0 : 1),
                      alignment: Alignment.center,
      ...
      wrappedTabs[index] = Opacity(
        ...
        child: IgnorePointer(
          ...
          child: InkWell(
            ...
            child: Container(
              padding: widget.outlineType == AnalysisTabBarOutlineType.filled
...
/// A page view that displays the widgets which corresponds to the currently
/// selected tab.
///
/// This widgets is typically used in conjunction with a [TDHorizontalTabBar].
///
...
class _TDHorizontalTabBarViewState extends State<TDHorizontalTabBarView> {
  ...
  void _handleTabControllerAnimationTick() {
    if (_warpUnderwayCount > 0 || !_controller!.indexIsChanging) {
      return;
    } // This widgets is driving the controller's animation.
...
class TabPageSelector extends StatelessWidget {
  /// Creates a compact widgets that indicates which tab has been selected.
