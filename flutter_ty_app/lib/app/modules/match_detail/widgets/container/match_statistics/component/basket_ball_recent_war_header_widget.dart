...
/// 篮球近期战绩头部组件
/// 用于显示近期战绩的标题和筛选标签（同场、同主客、同场/同主客）
class BasketBallRecentWarHeaderWidget {
  /// 构建近期战绩头部组件
  /// [name] 标题名称
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  name,
                  style: TextStyle(
                    fontSize: MatchStatisticMatchHistoryState.basketBallRecentWarHeaderTitleFontSize.sp,
                    color: Get.theme.tabPanelSelectedColor,
                    fontWeight: MatchStatisticMatchHistoryState.basketBallRecentWarHeaderTitleFontWeight,
                  ),
                ),
              ),
              _buildTabButton(
  ...
  static Widget _buildTabButton({
    ...
    final isSelected = controller.state.curSelectTab2.value == index;
    final width = isWide ? 100.w : 50.w;

    return InkWell(
      onTap: () {
        controller.setcurOtherSelectTab2(index);
      },
      child: isSelected
          ? BluetextNew(
              text,
