...
class EarlyAppointmentSettlementView extends StatelessWidget {
  ...
  Widget _submissionStatus(BuildContext context, bool enabled) {
    if (enabled) {
      ///预约提前兑现开启没有提交状态
      return _settlementRibbon(context).marginZero.marginOnly(
            bottom: 12.h,
          );
    } else {
      ///预约提前兑现开启已经提交状态
      return _settledRibbon(context).marginZero.marginOnly(
            bottom: 8.h,
          );
    }
  }
  ...
  Widget _earlySettlementButton(BuildContext context) {
    ...
    return InkWell(
      onTap: () => {
        unsettledBetsLogic.setEarlyAppointmentSettlement(
          index,
        ),
      },
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(
            color: data.earlyAppointmentSettlementType.value == 0
                ? Colors.white
                : isFootballBasketballTemplate ? HexColor(footballBasketballTemplateThemeColor) :Color(
                    context.isDarkMode ? 0xFF127DCC : 0xFF179CFF,
                  ),
            fontSize: 12.sp,
            fontFamily: 'PingFang SC',
                ...
                color: context.isDarkMode
                    ? Colors.white.withValues(alpha:
                        data.earlyAppointmentSettlementAmount.value.isEmpty
                            ? 0.20000000298023224
                            : 0.8999999761581421)
                    : Color(data.earlyAppointmentSettlementAmount.value.isEmpty
                        ? 0xFFAFB3C8
                        : 0xFF303442),
                fontSize: data.earlyAppointmentSettlementAmount.value.isEmpty
                    ? 12.sp
                    : 14.sp,
                fontFamily: 'PingFang SC',
                fontWeight: FontWeight.w500,
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text(
              FormatMoney.formatMoney2(BetsUtils.betReserveAmountMin(
                  unsettledBetsLogic.state.listData[index])),
              style: TextStyle(
                color: context.isDarkMode
        ...
        Row(
          ...
          children: [
            ...
            Text(
              FormatMoney.formatMoney2(BetsUtils.betReserveAmountMax(
                  unsettledBetsLogic.state.listData[index])),
              style: TextStyle(
                color: context.isDarkMode
                    ? Colors.white.withValues(alpha:0.699999988079071)
