 ...
 *
    ...
    【【【 AUTO_PARAGRAPH_TITLE 体育注单(阶段与时间)赛事比分展示视图Widget 】】】
 ...
 */
/// 赛事阶段（阶段与时间）、赛事比分
class RecordMatchInfo extends StatefulWidget {
  const RecordMatchInfo({
    super.key,
    required this.data,
  ...
  @override
  State<RecordMatchInfo> createState() => _RecordMatchInfoState();
}

class _RecordMatchInfoState extends State<RecordMatchInfo> {
  // 赛事阶段
  String betStage = "";
  ...
  @override
  void didUpdateWidget(covariant RecordMatchInfo oldWidget) {
    // 新旧列表对比
  ...
  void initFromApi(List<GetMatchLimitInfoByMidsDataData> result) {
    ...
    if (MatchUtil.is1Or2Csid(widget.data) && itemData != null) {
      // 如果包含当前赛事ID
      final matchId = itemData.matchId;
      final playId = itemData.playId.toString();
      final childPlayId = itemData.childPlayId.toString(); // 15分钟玩法子id
